...
Here are the essential Linux operators:

| Operator | Name | Use Case | Example |
|----------|------|----------|---------|
| `&` | Background | Run a command while continuing to use terminal | `ping google.com &` |
| `&&` | AND | Run commands sequentially (second only if first succeeds) | `cd /home && ls` |
| `>` | Redirect | Save output to a new file (overwrites) | `ls > files.txt` |
| `>>` | Append | Add output to an existing file | `echo "new line" >> files.txt` |
